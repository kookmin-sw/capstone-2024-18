...
class FaceLandmark(object):
    ...
    def show_result(self, image_path,image, landmark):
        min_y = sys.maxsize
        max_y = 0
        for idx,point in enumerate(landmark):
            ...
            if int(point[1])>max_y :
                max_y = int(point[1])
                max_x = int(point[0])
            if int(point[1])<min_y :
                min_y = int(point[1])
                min_x = int(point[0])

        face_h = max_y-min_y

        cv2.circle(image, center=(min_x,min_y),
                       color=(0, 255, 0), radius=2, thickness=-1)
        cv2.putText(image, str((min_x,min_y)), (min_x,min_y), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 0), 1) 
